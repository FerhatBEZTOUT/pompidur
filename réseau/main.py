...
while True:
    ...
    conn.settimeout(7)  # Timeout de 7 secondes
    ...
    if msg == 'HELLO':  # Il faut un "HELLO" pour continuer la communication, sinon la connexion se ferme instantanément
        ...
        if idcarte != NO_RSP:
            ...
            if re.match('^[0-9]{7}$', idcarte):
                query = 'SELECT * FROM carte WHERE id_carte=%s AND carte_active=TRUE AND date_exp>CURRENT_DATE'
                ...
                if cur.rowcount:
                    ...
                    if cur.rowcount:
                        ...
                        if userRow[0]:  # le champ utilisateur."hasfullaccess" == true
                            ...
                            if idterminal != NO_RSP:
                                query = 'SELECT id_salle FROM terminal WHERE id_terminal=%s'
                                cur = sql(query, (idterminal,))
                                r = cur.fetchone()
                                idSalle = r[0]
                                query = 'SELECT * FROM reserver WHERE id_adh=%s AND id_salle=%s AND dateTimeReserv<=NOW() AND dateTimeFin>=NOW()'
                                ...
                                if cur.rowcount:
                                    ...
                                    dateTimeReserv = r[2]   # recuperer le champ "dateTimeReserv"
                        ...
                        print(
                            "carte existe mais n'est associée à personne (Erreur critique, structure de la bdd a été modifiée)")
