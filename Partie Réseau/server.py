...
def recevoir(connexion):
    try:
        message = connexion.recv(net_config.tailleBuffer)
...
host = net_config.host  # adresse du serveur
port = net_config.port  # on reserve un port
try:
...
while True:
    ...
    if msg == 'HELLO':  # Il faut un "HELLO" pour continuer la communication, sinon la connexion se ferme instantanément
        ...
        print("Le client n'est pas courtois... Je ferme la connexion")
        envoyer(conn, "BYE")
        conn.close()
